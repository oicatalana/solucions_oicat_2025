...
$$f(i) = \sum_{j = 0}^{i} f(j-1) \cdot \text{es\\\_valid}[s_j][s_i]$$
...
$$ f(i) = \sum_{c = \text{'a'}}^{\text{'z'}} \text{acumulat}(c) \cdot \text{es\\\_valid}[c][s_i] $$
...
Només falta veure que podem mantenir els valors de $\text{acumulat}(c)$ actualitzats eficientment. Però això és senzill: cada cop que processem un caràcter $i$, afegim $f(i-1)$ a $\text{acumulat}(s_i)$ (important fer-ho abans de calcular $f(i)$, ja que és vàlid començar i acabar un segment a la posició $i$-èssima).
